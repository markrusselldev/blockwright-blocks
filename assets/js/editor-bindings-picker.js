...
( function ( wp ) {
	...
	var POST_FIELDS = {
		...
		'featured-image': { label: __( 'Post: Featured image', 'blockwright-blocks' ), forImage: true },
	};
	var SITE_FIELDS = {
	...
	function resolveField( select, source, field, context ) {
		...
		if ( 'blockwright/site' === source ) {
			var site = core.getEntityRecord( 'root', 'site' );
			if ( ! site ) {
				return undefined;
			}
			if ( 'title' === field ) {
		...
		if ( context && context.postId ) {
			record = core.getEditedEntityRecord( 'postType', context.postType, context.postId );
		} else {
		...
		var text = function ( value ) {
			if ( value && 'object' === typeof value && undefined !== value.rendered ) {
		...
		switch ( field ) {
			...
			case 'date':
				return record.date ? wp.date.dateI18n( wp.date.getSettings().formats.date, record.date ) : undefined;
			case 'author':
				var user = record.author ? core.getUser( record.author ) : null;
				return user ? user.name : undefined;
			case 'featured-image':
				var media = record.featured_media ? core.getMedia( record.featured_media ) : null;
	...
	function makeGetValues( sourceName ) {
		return function ( args ) {
			...
			Object.keys( args.bindings ).forEach( function ( attr ) {
				var field = args.bindings[ attr ].args && args.bindings[ attr ].args.field;
				var value = field ? resolveField( args.select, sourceName, field, args.context ) : undefined;
				if ( undefined !== value ) {
					values[ attr ] = value;
				}
			} );
	...
	function optionsFor( blockName ) {
		var options = [ { value: '', label: __( 'Static (not connected)', 'blockwright-blocks' ) } ];
		var isImage = 'core/image' === blockName;

		Object.keys( POST_FIELDS ).forEach( function ( field ) {
			if ( isImage === Boolean( POST_FIELDS[ field ].forImage ) ) {
				options.push( { value: 'blockwright/post|' + field, label: POST_FIELDS[ field ].label } );
			}
		} );
		if ( ! isImage ) {
			Object.keys( SITE_FIELDS ).forEach( function ( field ) {
				options.push( { value: 'blockwright/site|' + field, label: SITE_FIELDS[ field ].label } );
			} );
		}
		return options;
	}
	 ...
	 */
	var withDynamicContentPanel = createHigherOrderComponent( function ( BlockEdit ) {
		return function ( props ) {
			var attr = BOUND_ATTR[ props.name ];
			...
			var foreign = current && 0 !== currentValue.indexOf( 'blockwright/' );

			var preview = useSelect(
				function ( select ) {
					if ( ! currentValue || foreign ) {
						return undefined;
					}
					return resolveField( select, current.source, current.args.field, props.context );
				},
				[ currentValue, foreign, props.context && props.context.postId ]
			);

			var onChange = function ( next ) {
				var newBindings = Object.assign( {}, bindings );
				if ( next ) {
					var parts = next.split( '|' );
					newBindings[ attr ] = { source: parts[ 0 ], args: { field: parts[ 1 ] } };
				} else {
					delete newBindings[ attr ];
				}
				var newMetadata = Object.assign( {}, metadata, { bindings: newBindings } );
				if ( ! Object.keys( newBindings ).length ) {
					delete newMetadata.bindings;
				}
				props.setAttributes( {
					metadata: Object.keys( newMetadata ).length ? newMetadata : undefined,
				} );
			};

			var help;
			if ( foreign ) {
				help = __( 'Connected through another source; manage it in the Attributes panel.', 'blockwright-blocks' );
			} else if (
				currentValue &&
				...
				help = __( 'Previewing with your latest post.', 'blockwright-blocks' );
			} else if ( currentValue && undefined === preview ) {
				help = __( 'No value here yet (it fills in on a real post).', 'blockwright-blocks' );
			} else if ( ! currentValue ) {
				help = __( 'Fill this block from post or site data instead of typed content.', 'blockwright-blocks' );
			}
			...
			return el(
				...
				el(
					...
					el(
						PanelBody,
						{ title: __( 'Dynamic content', 'blockwright-blocks' ), initialOpen: true },
						el( SelectControl, {
	...
	addFilter( 'editor.BlockEdit', 'blockwright/dynamic-content-panel', withDynamicContentPanel );
} )( window.wp );

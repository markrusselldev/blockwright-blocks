...
const CORNER_CLASS = {
	'top-right': 'bw-stamp--tr',
	'top-left': 'bw-stamp--tl',
	'bottom-right': 'bw-stamp--br',
	'bottom-left': 'bw-stamp--bl',
};
const CORNERS = [
...
const stampClass = (a) =>
	'bw-stamp ' +
	(CORNER_CLASS[a.corner] || 'bw-stamp--tr') +
	' bw-stamp--border-' +
	(a.borderType || 'double');
function stampStyle(a) {
	const s = {
		'--bw-stamp-rot': (a.rotation == null ? 18 : a.rotation) + 'deg',
	};
	if (a.offsetX != null) {
	...
	return el(
		...
		el(
			...
			el(
				PanelBody,
				{
					title: __('Stamp', 'blockwright-blocks'),
					initialOpen: true,
				},
				el(SelectControl, {
					label: __('Corner', 'blockwright-blocks'),
					value: a.corner || 'top-right',
					options: CORNERS,
					onChange: (v) => set({ corner: v }),
					__nextHasNoMarginBottom: true,
				}),
				el(SelectControl, {
					label: __('Border', 'blockwright-blocks'),
					value: a.borderType || 'double',
					options: BORDERS,
					onChange: (v) => set({ borderType: v }),
					__nextHasNoMarginBottom: true,
				}),
				el(RangeControl, {
					label: __('Rotation (degrees)', 'blockwright-blocks'),
					value: a.rotation == null ? 18 : a.rotation,
					min: -45,
					max: 45,
					step: 1,
				...
				el(RangeControl, {
					label: __('Horizontal offset', 'blockwright-blocks'),
					help: __(
						'Negative overhangs the side edge.',
						'blockwright-blocks'
					),
					value: a.offsetX == null ? DEFAULT_X : a.offsetX,
					min: -40,
					max: 40,
					step: 1,
				...
				el(RangeControl, {
					label: __('Vertical offset', 'blockwright-blocks'),
					help: __(
						'Negative overhangs the top/bottom edge.',
						'blockwright-blocks'
					),
					value: a.offsetY == null ? DEFAULT_Y : a.offsetY,
					min: -40,
					max: 40,
					step: 1,
		...
		el(
			...
			Object.assign({}, blockProps, {
				...
				placeholder: __('Add text', 'blockwright-blocks'),
			})
		)
	);
}

function save({ attributes: a }) {
	...
	return el(
		RichText.Content,
		Object.assign({}, blockProps, { tagName: 'p', value: a.text })
	);
}

...
const rootClass = (a) =>
	'bw-icon-box is-position-' +
	(a.iconPosition || 'top') +
	' is-icon-' +
	(a.iconSize || 'large') +
	(a.alignment === 'center' ? ' is-align-center' : '') +
	// No verticalAlignment set = the default optical centering on the heading. An explicit
	// Top/Center/Bottom opts into aligning against the whole card (see style.scss).
	(isRow(a) && a.verticalAlignment
		? ' is-vertically-aligned-' + a.verticalAlignment
		: '');
...
function linkEl(a) {
	...
	return el(
		'a',
		{ className: 'bw-icon-box__link', href: a.linkUrl || '#' },
		a.linkText
	);
}
...
function Edit({ attributes: a, setAttributes: set }) {
	...
	const blockProps = useBlockProps({
		className: rootClass(a),
		style: isRow(a) ? { direction: 'ltr' } : undefined,
	});
	return el(
		Fragment,
		{},
		el(
			...
			isRow(a) &&
				el(BlockVerticalAlignmentControl, {
					// Unset (no button active) = optical centering on the heading (the default);
		...
		el(
			...
			el(
				PanelBody,
				{
					title: __('Icon Box', 'blockwright-blocks'),
					initialOpen: true,
				},
				el(Control, {
					value: a.icon,
				...
				el(SelectControl, {
					...
					options: [
						{
							value: 'top',
							label: __('Top (stacked)', 'blockwright-blocks'),
						},
						{
							value: 'left',
							label: __('Left', 'blockwright-blocks'),
						},
						{
							value: 'right',
							label: __('Right', 'blockwright-blocks'),
						},
					],
				...
				el(SelectControl, {
					...
					options: [
						{
							value: 'x-small',
							label: __('Extra small', 'blockwright-blocks'),
						},
						{
							value: 'small',
							label: __('Small', 'blockwright-blocks'),
						},
						{
							value: 'medium',
							label: __('Medium', 'blockwright-blocks'),
						},
						{
							value: 'large',
							label: __('Large', 'blockwright-blocks'),
						},
						{
							value: 'x-large',
							label: __('Extra large', 'blockwright-blocks'),
						},
					],
				...
				a.iconPosition === 'top' &&
					el(SelectControl, {
						...
						options: [
							...
							{
								value: 'center',
								label: __('Center', 'blockwright-blocks'),
							},
						],
				...
				el(TextControl, {
					label: __('Link text', 'blockwright-blocks'),
					value: a.linkText,
					onChange: (v) => set({ linkText: v }),
					__nextHasNoMarginBottom: true,
				}),
				el(TextControl, {
					label: __('Link URL', 'blockwright-blocks'),
					type: 'url',
					placeholder: 'https://',
					value: a.linkUrl,
		...
		el(
			...
			el(
				...
				el(RichText, {
					tagName: 'h3',
					className: 'bw-icon-box__heading',
					value: a.heading,
					allowedFormats: [],
				...
				el(RichText, {
					tagName: 'p',
					className: 'bw-icon-box__text',
					value: a.text,
					allowedFormats: ['core/bold', 'core/italic'],
					onChange: (v) => set({ text: v }),
					placeholder: __(
						'Describe this feature or benefit.',
						'blockwright-blocks'
					),
				}),
				linkEl(a)
